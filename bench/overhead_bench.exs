...
{:ok, composed} = Pool.start_link(config)
...
Benchee.run(
  %{
    ...
    "handled" => fn -> 100 = Handler.run(fun, opts) end,
    "pooled" => fn -> 100 = Pool.run(pool, fun, opts) end,
    "composed pools" => fn -> 100 = Pool.run(composed, fun, opts) end
  },
  time: 10
)
